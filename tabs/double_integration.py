...
def render_double_integration_tab(a, b, c, d):
    ...
    expr_input = st.text_input("f(x, y) =", "x*y + y**2")
...
def render_double_volume_tab(a, b, c, d):
    ...
    expr_input = st.text_input("f(x, y) =", "x**2 + y**2", key="double_volume_func")
    st.info("Esta función se integrará como volumen bajo la superficie f(x, y). Puedes usar sin(x), x*y^2, etc.")
