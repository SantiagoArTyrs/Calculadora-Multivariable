...
def show_triple_integral_volume_steps(f, x_lim, y_lim, z_lim):
    steps = []
    x0, x1 = x_lim
    y0, y1 = y_lim
    z0, z1 = z_lim

    steps.append(("Planteamiento de la integral de volumen:",
        f"\\iiint_{{E}} {sp.latex(f)} \\, dz \\, dy \\, dx = \\int_{{{x0}}}^{{{x1}}} \\int_{{{y0}}}^{{{y1}}} \\int_{{{z0}}}^{{{z1}}} {sp.latex(f)} \\, dz \\, dy \\, dx"))
    ...
    steps.append((f"Evaluando de z={z0} a z={z1}:", f"= {sp.latex(int_z)}"))
    ...
    steps.append((f"Evaluando de y={y0} a y={y1}:", f"= {sp.latex(int_y)}"))
    ...
    steps.append((f"Evaluando de x={x0} a x={x1}:", f"= {sp.latex(int_x)}"))

    steps.append(("🎉 Resultado final del volumen:", f"{sp.latex(int_x)}"))

    return steps, int_x


def show_double_integral_volume_steps(f, x_lim, y_lim):
    steps = []
    x0, x1 = x_lim
    y0, y1 = y_lim
    ...
    steps.append(("Antiderivada respecto a y:", f"\\int {sp.latex(f)} \\, dy = {sp.latex(raw_y)}"))

    int_y = sp.integrate(f, (y, y0, y1))
    steps.append((f"Evaluando de y={y0} a y={y1}:", f"= {sp.latex(int_y)}"))

    raw_x = sp.Integral(int_y, x)
    steps.append(("Antiderivada respecto a x:", f"\\int {sp.latex(int_y)} \\, dx = {sp.latex(raw_x)}"))
